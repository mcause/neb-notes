...
const largeNum = nums.find(num => num > 100);
...
const birthYear = people.sort((year1, year2) => year1.birthYear > year2.birthYear ? 1 : -1 );
...
const olderCars = cars.filter(carYear => carYear.make <= 1999);
...
const longNames = names.filter(fNames => fNames.length > 5);
...
const youngerAge = people1.filter(young => young.age > 20 && young.age < 35)
const youngest = youngerAge.sort((younger, older) => younger.age < older.age ? 1 : -1 );
